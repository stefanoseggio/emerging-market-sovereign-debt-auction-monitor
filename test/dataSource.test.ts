...
describe('fetchYearAuctions', () => {
    ...
    it('aborts a hung attempt at the tightened ~30s per-attempt timeout (not the old 60s) so the retry loop fits the run\'s real 300s timeoutSecs budget - see the REQUEST_TIMEOUT_MS doc comment\'s worst-case arithmetic', async () => {
        let callCount = 0;
        ...
        // Just under the new 30s boundary: the first attempt must still be the only one made - it
        // has not been aborted yet.
        await vi.advanceTimersByTimeAsync(29_000);
        expect(fetchMock).toHaveBeenCalledTimes(1);

        // Cross the 30s boundary, then allow the (sub-2s) backoff delay before the retry fires.
        // Under the OLD 60_000ms timeout this abort would not have fired yet at this point, the
        // second attempt would never have been made, and this assertion would fail - which is
