...
- **Dependency vulnerabilities** in `package.json`'s real dependency tree (`apify`, `xlsx`, and dev dependencies) are a real, ongoing concern. Dependabot version updates are configured and active (`.github/dependabot.yml`; live, currently-open PRs bump `typescript`, `eslint`, `vitest`, `fast-check`, and `@types/node`). GitHub's Dependabot *security* alerts and secret scanning are **not currently enabled** on this repository as of this audit — flagged for the maintainer to turn on in Settings → Code security, both free for public repos. Note `xlsx` is installed from SheetJS's own CDN tarball (`cdn.sheetjs.com`), not the npm registry — this is SheetJS's own current documented distribution channel for this package, not a fork or unofficial mirror.
